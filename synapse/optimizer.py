...
These decisions are written to the `dynamic_config` table (via the fixed ConfigManager),
which the main orchestrator reads on each cycle, allowing the pipeline to adapt in near real-time.
"""
import time
import logging
import sqlite3
...
# BUGFIX: The optimizer now uses the corrected ConfigManager, which writes
# changes to the database, allowing the main.py process to see them.
...
from config import (
    ...
    MAX_IMPLEMENTATION_WORKERS # Added to config for consistency
)
...
class AIMDController:
    ...
    def update(self, has_congestion: bool) -> bool:
        ...
        """
        # BUGFIX: The controller now needs to sync its view of the config
        # with the database at the start of its decision logic.
        ...
        if new_val != current_val:
            # This call now correctly writes to the database
...
def main() -> None:
    ...
    while True:
        try:
            # Sync config at the start of the loop
            config_manager.sync_from_db()
